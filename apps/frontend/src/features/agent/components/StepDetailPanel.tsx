  ...
  return (
    <section className="step-detail-panel">
      <div className="panel-heading compact">
        ...
        <span className={`run-pill step-${step.step_type}`}>#{step.step_index}</span>
      </div>
...
function FriendlyPayload({ step }: { step: AgentStep }) {
  ...
  if (step.tool_name === "scan_rules" && result) {
  ...
  if (step.tool_name === "search_regulation" && result) {
  ...
  if (args) {
    return <p className="agent-plain-summary">{Object.keys(args).length}개 입력 인자를 전달했습니다.</p>;
  }
